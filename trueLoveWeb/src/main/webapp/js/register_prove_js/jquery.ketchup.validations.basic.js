...
//匹配电话号码
$.fn.ketchup.validation('number', function(element, value) {
  if(/^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])[0-9]{8}$/.test(value)) return true;
  else return false;
});
...
//验证用户
$.fn.ketchup.validation('matchUser', function(element, value) {
  if(/^([0-9]+)|([A-Za-z]+)|(_)*|([u4e00-u9fa5])*$/.test(value)) return true;
  else return false;
});

//验证密码$("#input_pwd").val())
$.fn.ketchup.validation('matchPwd', function(element, value) {
	 if(/^([0-9]+)|([A-Za-z]+)|(_)*|([u4e00-u9fa5])*$/.test(value)) return true;
	 else return false;
});

//验证密码输入输入是否一致
$.fn.ketchup.validation('matchRePwd', function(element, value) {
	 if($("#input_repwd").val()==$("#input_pwd").val()) return true;
	 else return false;
});


$.fn.ketchup.validation('minlength', function(element, value) {
	  if (value.length < 6)
		return false;
	else
		return true;
...
$.fn.ketchup.validation('minlengthUser', function(element, value) {
	  if (value.length < 2)
		return false;
	else
		return true;
...
$.fn.ketchup.validation('maxlength', function(element, value) {
	if (value.length > 16)
		return false;
	else
		return true;
