...
function chkreg_email(regemail){
	if(!/^([0-9a-zA-Z\.\-_]+)[@]([0-9a-zA-Z\-]+)(\.[a-zA-Z0-9]+)+$/.test(regemail)) {
			return false;
	}
			return true;
}
...
/////////////////注册js/////////////////////////////////
$(function(){
	var err = '<img src="images/no.gif" style="vertical-align:middle;"/> ';
	var ok = '<img src="images/yes.gif" style="vertical-align:middle;"/> ';
	$("#email").focus(function(){
	       if($("#email").val()=='作为登录用户名及找回密码使用') $("#email").attr("value","");
		   if($("#email").val()=='该邮箱已被注册！') $("#email").attr("value","").css("color","#666");
	});
        //if($("#email").val()=='作为登录用户名及找回密码使用'){$("#email").focus(function(){$("#username").attr("value","");})}
	$("#email").blur(function(){
		...
		if(username == ''){
			...
			var changeUrl = '/ajax.php?n=register&h=checktel3&username_val='+username+"&r="+Math.random();
			$.get(changeUrl,function(str){
			if(str !== 'ok'){

				$("#validateemailID").html(err);
				$("#email").attr('value','该邮箱已被其他用户绑定').css("color","red");
			}else{
   ...
   $("#password").blur(function(){
		...
		if(pass == ""){
			...
			alert("密码必须为6到12位长度!");$("#pwdtip").html(err);$("#password").attr('value','');$("#passmessage").attr('value',"密码必须为6到12位长度!");
		}else{
			$("#pwdtip").html(ok);
		}
   })
   ...
   $("#telphone").focus(function(){
	       if($("#telphone").val()=='填入手机号码免费获取登录密码')$("#telphone").attr("value","");
		   if($("#telphone").val()=='该手机号已被注册！') $("#telphone").attr("value","").css("color","#666");
	});
   ...
   $("#telphone").blur(function(){
	   ...
	   if(!chk_phone1(tel)){
		   ...
		   var telurl = '/ajax.php?n=register&h=checktel&tel_val='+tel+"&r="+Math.random();

			$.get(telurl,function(str){
					if(str !== '"ok"'){
						$("#telphonemessage").attr('value','该号码已被其他用户绑定');
						$("#validatetelphoneID").html(err);
						$("#telphone").attr('value','该号码已被其他用户绑定').css("color","red");
					}else{
						$("#validatetelphoneID").html(ok);
						$("#telphonemessage").attr('value',"ok");
						$("#telphone").css("color","#666");
					}
				});
	   }
   })
